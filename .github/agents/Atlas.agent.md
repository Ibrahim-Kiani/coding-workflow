...
- Generate all plans in the `plans/` folder.
...
- You can invoke multiple subagents (up to 5) per phase if needed
...
6. **Pause for User Approval**: MANDATORY STOP. Wait for user to approve the plan or request changes using the #askQuestions tool. If changes requested, gather additional context and revise the plan.
