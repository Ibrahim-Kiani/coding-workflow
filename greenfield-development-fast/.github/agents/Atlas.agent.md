...
tools: ['read/readFile', 'search/listDirectory', 'agent', 'memory']
...
Each file in this snapshot follows this strict format:
...
When delegating parallel tasks, you MUST explicitly scope each agent to specific files to prevent conflicts. If a file has dependencies on other code files, Provide each file's COMPLETE CODE in the exact format as is provided to you to the subagent it concerns. Provide a high level overview of the files as well with the goal of discouraging the subagent to research the project itself.
